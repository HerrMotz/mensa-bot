...
def test_poll_mode_native_for_3_mensas(vm):
    assert vm.get_poll_mode() == "native"
...
def test_poll_mode_command_for_4_mensas(db):
    ...
    assert vm4.get_poll_mode() == "command"
...
@pytest.mark.asyncio
async def test_create_vote(vm):
    session_id, poll_mode, options, closes_at = await vm.create_vote()
    assert session_id > 0
...
async def test_record_native_ballot(vm):
    session_id, _, options, _ = await vm.create_vote()
...
async def test_update_ballot(vm):
    session_id, _, _, _ = await vm.create_vote()
...
@pytest.mark.asyncio
async def test_command_ballot_valid(vm, db):
    db.upsert_mensa("Carl-Zeiss-Promenade", "https://x.com", "CZP")
    mensas_4 = MENSAS + ["Carl-Zeiss-Promenade"]
    vm4 = VoteManager(
        db=db, mensas=mensas_4, max_poll_mensas=3,
...
@pytest.mark.asyncio
async def test_command_ballot_invalid_indices(vm, db):
    db.upsert_mensa("Carl-Zeiss-Promenade", "https://x.com", "CZP")
    mensas_4 = MENSAS + ["Carl-Zeiss-Promenade"]
    vm4 = VoteManager(
        db=db, mensas=mensas_4, max_poll_mensas=3,
...
async def test_restart_recovery(tmp_path):
    ...
    session_id, _, _, _ = await vm1.create_vote()
